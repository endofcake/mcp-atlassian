...
read-only and passes ``--ssl-certfile``/``--ssl-keyfile`` to the server, that
the readiness probe follows the listener to HTTPS, and that misconfigurations
fail the render loudly instead of yielding a pod that serves plaintext or
crashloops. The chart never creates a certificate. The render tests shell out
to ``helm`` when it is on PATH and skip otherwise; the text/values contract
tests always run.
"""
...
class TestServerTLSHelmContract:
    ...
    @requires_helm
    def test_render_wires_args_volume_mount_and_probe(self, tmp_path):
        """Enabling TLS wires the flags, a read-only Secret mount, and an
        ...
        assert volume["secret"]["defaultMode"] == 288  # octal 0440, not 0644
    ...
    def test_render_disabled_tls_omits_everything(self, tmp_path):
        """With TLS off, no flags, no mount, and no HTTPS probe scheme."""
        result = _helm_template(
            {
