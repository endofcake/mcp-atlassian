...
class TestBitbucketAvailability:
    ...
    def test_bitbucket_cloud_url_not_available(self, cloud_url, caplog):
        """A Cloud-shaped BITBUCKET_URL is unavailable (Data Center only).

        Keeps the gate aligned with BitbucketConfig.from_env, which rejects
        Cloud URLs. The gate must not advertise a service whose config load
        will fail.
        """
        with MockEnvironment.clean_env():
            import os

            os.environ["BITBUCKET_URL"] = cloud_url
            os.environ["BITBUCKET_OAUTH_ACCESS_TOKEN"] = "bb-token"

            result = get_available_services()
            assert result["bitbucket"] is False
            assert "only Bitbucket Data Center is supported" in caplog.text
