...
class TestListCommits:
    ...
    def test_unknown_merges_raises_without_request(self):
        """An unrecognised merges value raises before any request is made."""
        fetcher = _fetcher()
        with patch.object(fetcher._session, "get") as mock_get:
            with pytest.raises(
                ValueError, match="merges must be one of exclude, include, only"
            ):
                fetcher.list_commits("PROJ", "my-repo", merges="sometimes")
        mock_get.assert_not_called()

    @pytest.mark.parametrize("merges", ["", "   "])
    def test_blank_merges_is_dropped(self, merges):
        """A blank merges value is omitted so the server default applies."""
        fetcher = _fetcher()
        with patch.object(
            fetcher._session,
            "get",
            ...
            fetcher.list_commits("PROJ", "my-repo", merges=merges)
...
class TestGetCommitDotSegments:
    """A dot-segment commit id raises before any request is issued."""

    @pytest.mark.parametrize("sha", [".", ".."])
    def test_dot_commit_id_never_reaches_the_session(self, sha):
        fetcher = _fetcher()
        with patch.object(fetcher._session, "get") as mock_get:
            with pytest.raises(ValueError, match="commit_id must"):
                fetcher.get_commit("PROJ", "repo", sha)
        mock_get.assert_not_called()
