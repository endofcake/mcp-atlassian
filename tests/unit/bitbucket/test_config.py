...
class TestBitbucketConfigFromEnv:
    ...
    def test_from_env_byo_token_takes_precedence_over_client_creds(self):
        """With both a BYO token and client creds set, the BYO token wins.

        Pins the get_oauth_config_from_env ordering the loader relies on.
        """
        with patch.dict(
            os.environ,
            {
    ...
    def test_from_env_global_proxy_fallback(self):
        """Global proxy vars apply when no BITBUCKET_* overrides are set."""
        with patch.dict(
            os.environ,
            {
                "BITBUCKET_URL": "https://bitbucket.corp.example.com",
    ...
    @pytest.mark.parametrize("timeout_value", ["not-a-number", "0", "-5"])
    def test_from_env_invalid_timeout_falls_back_with_warning(
        self, timeout_value, caplog
    ):
        """A non-numeric or non-positive BITBUCKET_TIMEOUT falls back and warns."""
        with patch.dict(
            os.environ,
            {
                "BITBUCKET_URL": "https://bitbucket.corp.example.com",
                "BITBUCKET_OAUTH_ACCESS_TOKEN": "byo-token",
                "BITBUCKET_TIMEOUT": timeout_value,
            },
            clear=True,
        ):
            config = BitbucketConfig.from_env()
...
class TestBitbucketConfigIsAuthConfigured:
    ...
    @staticmethod
    def _oauth(client_id: str, client_secret: str) -> BitbucketConfig:
    ...
    def test_empty_client_credentials_count_as_configured(self, caplog):
        """Empty client id and secret mean per-request bearer tokens."""
        with caplog.at_level("WARNING"):
            assert self._oauth("", "").is_auth_configured() is True
        assert "Incomplete Bitbucket OAuth configuration" not in caplog.text

    @pytest.mark.parametrize(
        ("client_id", "client_secret"),
        [("c", ""), ("", "s")],
        ids=["id-only", "secret-only"],
    )
    def test_partial_client_credentials_warn_and_fail(
        self, caplog, client_id, client_secret
    ):
        """One credential without the other is incomplete and logs the warning."""
        with caplog.at_level("WARNING"):
            result = self._oauth(client_id, client_secret).is_auth_configured()
        assert result is False
        assert "Incomplete Bitbucket OAuth configuration" in caplog.text

    @pytest.mark.parametrize(
        ("access_token", "expected"), [("byo-token", True), ("", False)]
    )
    def test_byo_access_token_branch(self, caplog, access_token, expected):
        """A BYO token is configured when present; an empty one warns."""
        config = BitbucketConfig(
            url="https://bitbucket.corp.example.com",
            auth_type="oauth",
