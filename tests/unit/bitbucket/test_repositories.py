...
class TestListRepositoriesNameFilter:
    ...
    def test_name_filtered_path_is_single_window(self):
        ...
        with patch.object(
            ...
            "get",
            return_value=_page_response(
                [_repo("api")], is_last_page=False, next_page_start=50
            ),
        ) as mock_get:
        ...
        assert page.next_page_start == 50
        assert page.is_last_page is False
        assert page.truncated is True
...
class TestListRepositoriesDotSegments:
    """A dot-segment project key raises before any request is issued.

    ``quote`` leaves periods unencoded, and the HTTP layer would normalise
    ``/projects/../repos`` into the cross-project ``/repos`` endpoint.
    """

    @pytest.mark.parametrize("key", [".", ".."])
    def test_dot_project_key_never_reaches_the_session(self, key):
        fetcher = _fetcher()
        with patch.object(fetcher._session, "get") as mock_get:
            with pytest.raises(ValueError, match="project_key must"):
                fetcher.list_repositories(key)
        mock_get.assert_not_called()
