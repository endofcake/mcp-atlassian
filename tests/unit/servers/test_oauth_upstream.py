...
class TestProxyUpstreamProvider:
    ...
    def test_warns_when_enabled_without_any_client_id(self, monkeypatch, caplog):
        # An enabled proxy with no client id silently degrades to no auth
        # surface; the operator must see why the proxy was not built.
        monkeypatch.setenv("ATLASSIAN_OAUTH_PROXY_ENABLE", "true")
        with caplog.at_level("WARNING"):
            assert proxy_upstream_provider() is None
        assert "no upstream OAuth client id is configured" in caplog.text

    def test_no_warning_when_proxy_disabled(self, caplog):
        with caplog.at_level("WARNING"):
            assert proxy_upstream_provider() is None
        assert "no upstream OAuth client id" not in caplog.text
...
class TestResolveBitbucketUpstream:
    ...
    def test_missing_scope_fails_closed_with_no_atlassian_fallback(self, monkeypatch):
        # Bitbucket Data Center has no default OAuth scope, and a scope string
        # meant for a different provider must not leak in, so a fully
        # configured Bitbucket client without BITBUCKET_OAUTH_SCOPE refuses to
        # start rather than sending scope-less authorize requests upstream.
        self._set_bitbucket_env(monkeypatch)
        monkeypatch.delenv("BITBUCKET_OAUTH_SCOPE", raising=False)
        monkeypatch.setenv("ATLASSIAN_OAUTH_SCOPE", "read:jira-work")

        with pytest.raises(RuntimeError, match="BITBUCKET_OAUTH_SCOPE"):
            resolve_proxy_upstream()
...
def test_client_secret_not_exposed_in_repr(monkeypatch):
    monkeypatch.setenv("ATLASSIAN_OAUTH_PROXY_ENABLE", "true")
    monkeypatch.setenv("ATLASSIAN_OAUTH_CLIENT_ID", "client-id")
    monkeypatch.setenv("ATLASSIAN_OAUTH_CLIENT_SECRET", "top-secret-value")
    ...
    assert upstream is not None
    assert "top-secret-value" not in repr(upstream)
