...
description: "Overview of all 80 MCP tools for Jira, Confluence, and Bitbucket Data Center — organized by category with quick links"
---

MCP Atlassian provides **80 tools** for interacting with Jira, Confluence, and Bitbucket Data Center. Tools are organized by category below.
...
## Bitbucket Data Center Tools

<CardGroup cols={2}>
  <Card title="Bitbucket" icon="code-branch" href="/docs/tools/bitbucket">
    Projects, repositories, and the pull-request review loop (read-only, OAuth)
  </Card>
</CardGroup>
...
**Bitbucket Data Center Toolsets (3):** all off by default; the `bitbucket_pull_requests`
toolset includes write tools (`bitbucket_add_comment`, `bitbucket_set_review_status` — both
gated by `READ_ONLY_MODE`).

| Toolset | Core | Tools |
|---------|:----:|-------|
| `bitbucket_projects` | No | `bitbucket_list_projects` |
| `bitbucket_repositories` | No | `bitbucket_list_repositories` |
| `bitbucket_pull_requests` | No | `bitbucket_list_pull_requests`, `bitbucket_get_pull_request`, `bitbucket_get_pull_request_diff`, `bitbucket_get_pull_request_activities`, `bitbucket_get_pull_request_comments`, `bitbucket_add_comment`, `bitbucket_set_review_status` |
