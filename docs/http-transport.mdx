...
```bash
# Using uvx
uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --transport streamable-http --stateless --port 9000 -vv
...
STATELESS=true uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --transport streamable-http --port 9000
```
...
<Tabs>
  <Tab title="SSE Transport">
    ```bash
    # Using uvx
    uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --transport sse --port 9000 -vv
  ...
  <Tab title="Streamable-HTTP">
    ```bash
    # Using uvx
    uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --transport streamable-http --port 9000 -vv
...
<Tabs>
  ...
  <Tab title="Bitbucket DC (OAuth)">
    Bitbucket Data Center accepts an OAuth 2.0 access token issued by the
    Bitbucket instance, sent as a bearer token. Bitbucket ignores
    `X-Atlassian-Cloud-Id`.

    Server-side, set the Bitbucket URL and enable user-provided tokens. No
    server credential is needed:
    ...
    ```json
    {
      "mcpServers": {
        "mcp-atlassian-service": {
          "url": "http://localhost:9000/mcp",
          "headers": {
            "Authorization": "Bearer <BITBUCKET_DC_OAUTH_ACCESS_TOKEN>"
          }
        }
      }
    }
    ...
    Alternatively, enable the [OAuth proxy for Bitbucket](/docs/authentication#bitbucket-data-center)
    and let MCP clients obtain the token through the proxy.
  </Tab>
</Tabs>
