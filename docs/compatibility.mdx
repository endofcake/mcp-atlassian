...
Bitbucket support is a property of the Atlassian side rather than the AI platform: every platform above reaches Bitbucket Data Center through the same tools, and Bitbucket Cloud is not supported by any of them. See [Cloud vs Server/Data Center](#cloud-vs-serverdata-center) below.
...
MCP Atlassian supports both Atlassian Cloud and Server/Data Center deployments for Jira and Confluence, but some features differ between platforms. Bitbucket is supported on Data Center only.
...
Bitbucket Data Center accepts OAuth 2.0 only (client credentials from an incoming application link, the Bitbucket-side registration that issues them, or a bring-your-own access token); there is no API-token, password, or PAT path, and Bitbucket Cloud is not supported.
