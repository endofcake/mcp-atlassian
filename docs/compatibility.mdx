...
Every platform above reaches Bitbucket Data Center through the same tools. See [Cloud vs Server/Data Center](#cloud-vs-serverdata-center) below.
...
in upstream sooperset/mcp-atlassian [issue #1541](https://github.com/sooperset/mcp-atlassian/issues/1541).
...
| `MCP_READINESS_BITBUCKET_URL` | Optional Bitbucket DC staging URL; setting it adds the Bitbucket read |
| `MCP_READINESS_BITBUCKET_ACCESS_TOKEN` | OAuth 2.0 access token issued by that Bitbucket instance |
| `MCP_READINESS_BITBUCKET_PROJECT_KEY` | Project key of the repository approved for a read probe |
| `MCP_READINESS_BITBUCKET_REPOSITORY_SLUG` | Slug of that repository |
...
The Bitbucket read is opt-in. When `MCP_READINESS_BITBUCKET_URL` is set, the
other three Bitbucket variables become required and map to `BITBUCKET_URL`
and `BITBUCKET_OAUTH_ACCESS_TOKEN` in the child environment. The probe then
reads the repository's default branch with `bitbucket_get_default_branch`
one pacing interval after the Confluence read, and the output's
`staging_profile` reads `jira-dc-confluence-dc-pat+bitbucket-dc-oauth`.

The profile fails closed: it forces read-only mode, exposes only
`jira_get_issue`, `confluence_get_page`, and (when configured)
`bitbucket_get_default_branch`, disables retries, limits concurrency
...
<Steps>
  ...
  <Step title="Verify tool discovery">
    If Copilot reports "Retrieved 0 tools", ensure you are running a current build from this repository. Current releases run on FastMCP v3 (`>=3.2.4,<4.0.0`) while exposing the standard MCP protocol over stdio and HTTP, so MCP clients do not need FastMCP installed. If an older client still cannot discover tools, update the client to a current MCP-compatible release.
  </Step>

  <Step title="Docker alternative">
    When using Docker, expose via stdio (not HTTP). Build the image first as described in [Installation](/docs/installation#docker):
...
If you encounter schema errors, ensure you are running the latest build from this repository (`uvx` caches Git installs, so refresh explicitly):

```bash
uvx --refresh --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --help
```

### ChatGPT

ChatGPT's MCP integration has reported vague "violates guidelines" errors. This may be related to schema validation but no specific diagnostics are available. If you encounter this, please [open an issue](https://github.com/endofcake/mcp-atlassian/issues) with the exact error message.
...
MCP Atlassian supports both Atlassian Cloud and Server/Data Center deployments for Jira and Confluence, but some features differ between platforms.
...
Bitbucket Data Center authenticates with OAuth 2.0. See [Bitbucket Data Center](/docs/authentication#bitbucket-data-center).
...
| `bitbucket_*` (37 tools) | No | Yes (Data Center) | Projects, repositories, refs, commits, build statuses, ref comparison, source browsing, pull requests, current user |
