...
<Tabs>
  ...
  <Tab title="Bitbucket Only">
    Bitbucket Data Center with an access token from its application link:
      ...
      "mcpServers": {
        "mcp-atlassian": {
          ...
          "env": {
            ...
            "BITBUCKET_OAUTH_ACCESS_TOKEN": "your_bitbucket_oauth_access_token"
          }
        }
      }
    }
    ...
    For per-request tokens and the OAuth proxy, see
    [Bitbucket Data Center](/docs/authentication#bitbucket-data-center).
  </Tab>
</Tabs>
...
| `BITBUCKET_URL` | Bitbucket Data Center base URL |
| `BITBUCKET_SSL_VERIFY` | SSL verification (`true`/`false`, default: `true`) |
| `JIRA_TIMEOUT` | Jira request timeout in seconds (default: `75`) |
| `CONFLUENCE_TIMEOUT` | Confluence request timeout in seconds (default: `75`) |
| `BITBUCKET_TIMEOUT` | Bitbucket request timeout in seconds (default: `75`) |
...
Bitbucket authenticates with OAuth 2.0. Its `BITBUCKET_OAUTH_*` variables are
described under [Bitbucket Data Center](/docs/authentication#bitbucket-data-center).
...
| `BITBUCKET_PROJECTS_FILTER` | Allowlist of Bitbucket project keys applied to project discovery and listing (`bitbucket_list_projects`). Other tools accept any project key | `PROJ,DEVOPS` |
...
| `JIRA_FETCHER_MAX_WORKERS` | Maximum concurrent Jira tool calls offloaded to worker threads (default: `8`) |
| `BITBUCKET_FETCHER_MAX_WORKERS` | Maximum concurrent Bitbucket tool calls offloaded to worker threads (default: `8`) |
...
See [.env.example](https://github.com/endofcake/mcp-atlassian/blob/main/.env.example) for all available options.
...
| `ATLASSIAN_MAX_CONCURRENT_REQUESTS` | Process-wide cap for concurrent requests across all enabled services (Jira, Confluence, and Bitbucket). `0` or unset disables the cap. |
...
| `BITBUCKET_HTTP_PROXY`, `BITBUCKET_HTTPS_PROXY`, `BITBUCKET_SOCKS_PROXY`, `BITBUCKET_NO_PROXY` | Bitbucket-specific explicit proxy overrides |
| `BITBUCKET_PROXY_WPAD_ENABLE`, `BITBUCKET_PROXY_WPAD_URL` | Bitbucket-specific PAC/WPAD overrides |
...
PAC/WPAD support is provided by the optional `wpad` extra. Install it before
enabling the feature, for example with
`uvx --from "mcp-atlassian[wpad] @ git+https://github.com/endofcake/mcp-atlassian" mcp-atlassian`
or `uv sync --extra wpad` in a checkout. The Dockerfile in this repository
installs this extra.
...
| `BITBUCKET_CUSTOM_HEADERS` | Custom headers for Bitbucket requests |
...
| `BITBUCKET_PASSTHROUGH_HEADERS` | Incoming MCP request header names to pass through to Bitbucket requests |
...
Some header names are ignored even when listed: `Authorization`,
`Proxy-Authorization`, `Host`, and hop-by-hop headers (such as `Connection`,
`Transfer-Encoding`, `Keep-Alive`) are dropped with a warning, because
forwarding them would replace the session's own upstream credential or corrupt
the upstream request. In external auth mode the incoming bearer is the intended
upstream credential, so `Authorization` is forwarded there.
...
Each `MCP_ALLOWED_URL_DOMAINS` entry is a suffix match that also trusts every
subdomain beneath it, so list full hostnames such as `jira.corp.example.com`.
The configured `JIRA_URL`, `CONFLUENCE_URL`, and `BITBUCKET_URL` hosts are
trusted as exact matches. A deployment whose load balancer redirects to
per-node private hostnames (for example `node1.jira.internal`) must add those
hostnames to `MCP_ALLOWED_URL_DOMAINS`, or requests to them are refused as
...
```bash
uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --enabled-tools "confluence_search,jira_get_issue"
```
...
Command-line alternative:
```bash
uvx --from git+https://github.com/endofcake/mcp-atlassian mcp-atlassian --toolsets "default,jira_agile"
```
