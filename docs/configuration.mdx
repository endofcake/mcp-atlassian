...
the upstream request. `Authorization` is forwarded only in external auth mode,
where the incoming bearer is the intended upstream credential. Deployments
that previously listed `Authorization` for a non-external session were
forwarding the wrong token; that configuration is now ignored rather than
honored.
...
subdomain beneath it — list full hostnames (`jira.corp.example.com`), never a
bare apex or label. The configured `JIRA_URL` / `CONFLUENCE_URL` /
`BITBUCKET_URL` hosts, by contrast, are trusted exactly, not as suffixes: a
deployment whose load balancer redirects to per-node private hostnames (for
example `node1.jira.internal`) must add those hostnames to
`MCP_ALLOWED_URL_DOMAINS`, or requests to them are refused with a non-global
address rejection.
