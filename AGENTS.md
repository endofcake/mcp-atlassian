...
| `  ├─ confluence/` | Confluence client + 10 mixins (pages, search, analytics, …) |
...
- **Mixin composition**: `JiraFetcher` composes 21 mixins, `ConfluenceFetcher` composes 10. Client inheritance is transitive through mixins. (Upstream `sooperset/mcp-atlassian` is at 8 Confluence mixins; community/main carries `RestrictionsMixin` and `TemplatesMixin` from pending upstream PRs sooperset/mcp-atlassian#1178 and #1180.)
