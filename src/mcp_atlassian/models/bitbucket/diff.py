...
``RestDiffLine`` shape rather than raw unified-diff text. A per-file line budget
...
def _entries(container: dict[str, Any], key: str) -> list[dict[str, Any]]:
    """Return a nested diff collection (``hunks``, ``segments``, ``lines``).

    An absent or null key is an empty collection (a binary or server-truncated
    file carries no ``hunks``). A present value that is neither null nor a
    list, or a list with a non-object entry, is a response-shape error:
    dropping the entry would report a corrupt diff as a complete one.

    Args:
        container: The enclosing raw object.
        key: The collection key to read.

    Returns:
        The list of raw entry objects.

    Raises:
        ValueError: If the value is present, not null, and not a list of
            objects.
    """
    ...
    if not isinstance(raw, list):
        raise ValueError(
            "Bitbucket returned an unexpected diff response shape: "
            f"'{key}' is {type(raw).__name__}, not a list. Refusing to "
            "report the malformed body as a complete diff."
        )
    if not all(isinstance(entry, dict) for entry in raw):
        raise ValueError(
            "Bitbucket returned an unexpected diff response shape: expected "
            f"every '{key}' entry to be an object. Refusing to report the "
            "malformed body as a complete diff."
        )
    return raw
...
def _count_hunk_lines(hunk: dict[str, Any]) -> int:
    ...
    Raises:
        ValueError: If the hunk's ``segments`` or a segment's ``lines`` is
            malformed (see :func:`_entries`).
    """
    return sum(len(_entries(seg, "lines")) for seg in _entries(hunk, "segments"))
...
class BitbucketDiffLine(ApiModel):
    ...
    def from_api_response(
        ...
        model = cls.__name__
        return cls(
            line=_opt_str(data, "line", model=model) or EMPTY_STRING,
            source=_opt_int(data, "source", model=model),
            destination=_opt_int(data, "destination", model=model),
            truncated=_opt_bool(data, "truncated", model=model),
        )
...
class BitbucketDiffSegment(ApiModel):
    ...
    def from_api_response(
        ...
        Raises:
            ValueError: If ``lines`` is present and is not a list of objects,
                or a scalar field has a value of the wrong type.
        """
        if not isinstance(data, dict) or not data:
            return cls()
        return cls._build(data, _entries(data, "lines"))

    @classmethod
    def _build(
        cls, data: dict[str, Any], lines: list[dict[str, Any]]
    ) -> "BitbucketDiffSegment":
        """Build a segment from its raw object and the raw lines to keep."""
        model = cls.__name__
        return cls(
            type=_opt_str(data, "type", model=model),
            lines=[BitbucketDiffLine.from_api_response(line) for line in lines],
            truncated=_opt_bool(data, "truncated", model=model),
        )
...
class BitbucketDiffHunk(ApiModel):
    ...
    def _build(
        ...
        Raises:
            ValueError: If ``segments`` or a segment's ``lines`` is present and
                is not a list of objects, or a scalar field has a value of the
                wrong type.
        """
        segments: list[BitbucketDiffSegment] = []
        used = 0
        ...
        truncated = False
        budget = remaining
        for raw_segment in _entries(data, "segments"):
            lines = _entries(raw_segment, "lines")
            if budget is None:
                kept = lines
            elif budget <= 0:
                omitted += len(lines)
            ...
            if budget is not None:
                budget -= len(kept)
            segments.append(BitbucketDiffSegment._build(raw_segment, kept))
        model = cls.__name__
        hunk = cls(
            context=_opt_str(data, "context", model=model),
            source_line=_opt_int(data, "sourceLine", model=model),
            source_span=_opt_int(data, "sourceSpan", model=model),
            destination_line=_opt_int(data, "destinationLine", model=model),
            destination_span=_opt_int(data, "destinationSpan", model=model),
            segments=segments,
            truncated=_opt_bool(data, "truncated", model=model),
        )
    ...
    def from_api_response(
        ...
        Returns:
            A BitbucketDiffHunk instance (a default instance for empty/non-dict
            input).
...
class BitbucketFileDiff(ApiModel):
    ...
    def from_api_response(
        ...
        **kwargs: Any,
        ...
        Raises:
            ValueError: If ``hunks``, a hunk's ``segments``, or a segment's
                ``lines`` is present and is not a list of objects, or a scalar
                field has a value of the wrong type.
        """
        if not isinstance(data, dict) or not data:
            return cls()
        ...
        for raw_hunk in _entries(data, "hunks"):
            ...
            # A hunk is kept even when it carries no lines, so its ``@@``
            # coordinates survive.
            hunks.append(hunk)
        ...
        return cls(
            ...
            binary=_opt_bool(data, "binary", model=model),
            hunks=hunks,
            server_truncated=_opt_bool(data, "truncated", model=model) is True,
...
class BitbucketPullRequestDiff(ApiModel):
    ...
    def from_api_response(
        ...
        **kwargs: Any,
        ...
        Returns:
            A BitbucketPullRequestDiff instance (an empty diff for a non-dict or
            empty body).

        Raises:
            ValueError: If a non-empty body has no ``diffs`` list, if any
                ``diffs`` entry is not an object, or if a nested ``hunks``,
                ``segments``, or ``lines`` collection is malformed (see
                :meth:`BitbucketFileDiff.from_api_response`). A parse failure
                must surface as an error rather than read as an empty
                (no-change) diff.
        """
        if not isinstance(data, dict) or not data:
            return cls()
        ...
        if not isinstance(raw_files, list):
            raise ValueError(
                "Bitbucket returned an unexpected diff response shape: 'diffs' "
                "is absent or not a list in a non-empty body. Refusing to "
                "report the malformed body as an empty diff."
            )
        if not all(isinstance(f, dict) for f in raw_files):
            raise ValueError(
                "Bitbucket returned an unexpected diff response shape: expected "
                "every 'diffs' entry to be an object. Refusing to drop malformed "
                "file diffs."
            )
        ...
        truncated = (
            _opt_bool(data, "truncated", model=cls.__name__) is True
            or files_truncated
