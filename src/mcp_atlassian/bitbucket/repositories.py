...
class ReposMixin(BitbucketClient):
    ...
    def list_repositories(
        ...
        *,
        ...
        ``start``/``limit``); one window per call,
        resumable via the returned ``next_page_start``.

        With a non-blank ``name`` it routes to the cross-project
        ``GET /rest/api/1.0/repos`` search, scoped to this project via the
        ``projectkey`` query param, with ``name`` as a server-side filter. Both
        forms return the same
        ...
        limit = max(
            1,
            min(
                clamp_limit(limit, context="bitbucket.list_repositories"),
                MAX_REPOS_LIMIT,
            ),
        )
        params: dict[str, Any] | None
        if name and name.strip():
            # Cross-project search scoped to this project: project_key rides
            # as a query param (requests URL-encodes it).
            path = "/repos"
            params = {"projectkey": key, "name": name.strip()}
        else:
            # project_key is caller-supplied and goes into the request path, so
            # it is validated and percent-encoded (no dot segments, no
            # unescaped slashes) to prevent path traversal or query-string
            # injection into the Bitbucket request.
            encoded_key = self._encode_segment(
                key, name="project_key", what="project key"
            )
            path = f"/projects/{encoded_key}/repos"
            params = None
