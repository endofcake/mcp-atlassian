...
# The three values the ``merges`` query param accepts; normalized through
# BitbucketClient._enum_param (a blank value is dropped, any other value raises).
...
class CommitsMixin(BitbucketClient):
    ...
    def _commit_list_params(
        ...
        Every filter is applied server-side. ``merges`` is normalized to the
        three known values; a blank value is dropped and an unrecognised value
        raises (see :meth:`BitbucketClient._enum_param`). The string-typed
        bool params (``followRenames``, ``ignoreMissing``) are sent as the
        lowercase strings the instance expects, because ``requests`` renders a
        raw Python bool as the capitalized ``"True"``/``"False"`` the instance
        rejects.
        ...
        Raises:
            ValueError: If ``follow_renames`` is set without a single-file
                ``path``, or ``merges`` is not a recognised value. The instance
                would reject either with an opaque 400, so both are caught
        ...
        merge_mode = BitbucketClient._enum_param(
            merges, name="merges", allowed=_MERGES_VALUES
        )
        if merge_mode is not None:
            params["merges"] = merge_mode
    ...
    def list_commits(
        ...
        *,
        ...
        commits`` (paged with ``start``/``limit``); one window per
        call, resumable via the returned ``next_page_start``. The history
        ...
        Raises:
            ValueError: If a segment is blank, a filter is invalid (see
                :meth:`_commit_list_params`), a page is misshaped, or the request
                fails (see :meth:`BitbucketClient._get`).
        ...
        limit = max(
            1,
            min(
                clamp_limit(limit, context="bitbucket.list_commits"), MAX_COMMITS_LIMIT
            ),
        )
    ...
    def get_commit(
        ...
        sha = self._encode_segment(commit_id, name="commit_id", what="commit id")
        path = f"{base}/{sha}"
        data = self._get(path)
    ...
    def get_pull_request_commits(
        ...
        *,
        ...
        ``next_page_start``. This endpoint accepts only ``start``/``limit``
        (no filters).
        ...
        limit = max(
            1,
            min(
                clamp_limit(limit, context="bitbucket.get_pull_request_commits"),
                MAX_COMMITS_LIMIT,
            ),
        )
        page = self._fetch_page(
