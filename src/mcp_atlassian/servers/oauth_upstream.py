...
def proxy_upstream_provider() -> ProxyProvider | None:
    ...
    if has_atlassian:
        return "atlassian"
    logger.warning(
        "OAuth proxy is enabled (%s=true) but no upstream OAuth client id is "
        "configured — set ATLASSIAN_OAUTH_CLIENT_ID (or JIRA_/CONFLUENCE_"
        "OAUTH_CLIENT_ID) or BITBUCKET_OAUTH_CLIENT_ID. The proxy will not "
        "be built and the server starts without it.",
...
def _resolve_atlassian_upstream() -> ProxyUpstream | None:
    ...
    if not instance_url or not client_id or not redirect_uri:
        logger.warning(
            "OAuth proxy requested but configuration is incomplete: need an "
            "instance URL, an OAuth client id, and a redirect URI."
        )
        return None
...
def _resolve_bitbucket_upstream() -> ProxyUpstream | None:
    ...
    Bitbucket Data Center is self-hosted, so its OAuth provider uses the Data
    Center endpoint shape. The redirect URI falls back to the shared
    ``ATLASSIAN_OAUTH_REDIRECT_URI`` (the redirect URI addresses this
    server). The scope env is Bitbucket-specific with no shared fallback, so a
    ...
    if not instance_url or not client_id or not redirect_uri:
        logger.warning(
            "OAuth proxy requested for Bitbucket but configuration is "
            "incomplete: need BITBUCKET_URL, an OAuth client id, and a "
            "redirect URI."
        )
        return None

    # Cloud is not handled. A Cloud-looking BITBUCKET_URL would resolve to
    ...
    if is_atlassian_cloud_url(instance_url) or is_bitbucket_cloud_url(instance_url):
        logger.warning(
            ...
            "Bitbucket Data Center OAuth endpoints only, so it refuses the "
            "host rather than build Data Center endpoints against Cloud.",
            instance_url,
        )
        return None
    ...
    scopes = _parse_scopes(scope_env)
    if not scopes:
        # Bitbucket Data Center has no usable default scope (the Jira/Confluence
        # Data Center default does not exist there), and an authorize request
        # without scopes fails only later, upstream, with a less clear error.
        # An operator who configured full Bitbucket client credentials intends
        # the proxy, so fail closed at startup instead.
        raise RuntimeError(
            "BITBUCKET_OAUTH_SCOPE is required when the OAuth proxy fronts "
            "Bitbucket: Bitbucket Data Center has no default OAuth scope. Set "
            "it to the scopes granted to the incoming application link, e.g. "
            "PUBLIC_REPOS, REPO_READ, REPO_WRITE, REPO_ADMIN or PROJECT_ADMIN."
        )
...
def resolve_proxy_upstream() -> ProxyUpstream | None:
    ...
    Raises:
        RuntimeError: If credentials for more than one provider family are set
            (see :func:`proxy_upstream_provider`), or if the Bitbucket provider
            is configured without ``BITBUCKET_OAUTH_SCOPE``.
    """
